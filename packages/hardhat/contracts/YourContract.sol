...
contract YourContract {
    ...
    event Withdrawn(address indexed beneficiary, uint256 amount, address indexed token);
    event TokenWithdrawn(address indexed beneficiary, uint256 amount, address indexed token);
    ...
    function depositAndSplitETH(address[] calldata recipients, uint256[] calldata amounts) external payable {
        ...
        require(msg.value == totalAmount, "Sent value does not match total split amount");
    ...
    function depositAndSplitToken(address token, address[] calldata recipients, uint256[] calldata amounts) external {
        require(recipients.length == amounts.length, "Recipients and amounts length mismatch");

        IERC20 erc20 = IERC20(token);
        for (uint256 i = 0; i < recipients.length; i++) {
            require(erc20.transferFrom(msg.sender, address(this), amounts[i]), "Transfer failed");
